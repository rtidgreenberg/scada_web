"""WIS-compatible DynamicData → JSON mapping.

Transforms the raw output of DynamicData.to_json() to match RTI Web
Integration Service behavior:
  ...
  - char[N] arrays: to_json() emits ["R","U","N","","","..."] — this module
    converts them to a single NUL-trimmed string "RUN" matching WIS output.
...
def _transform(obj: Any) -> Any:
    """Recursively transform a parsed JSON tree to WIS-compatible form."""
...
def sample_to_dict(data: dds.DynamicData) -> dict[str, Any]:
    """Convert a DynamicData sample to a WIS-compatible dict.

#!/usr/bin/env bash
# start-select.sh — Start the SCADA selector (scada_selector)
#
...
# the scada_selector binary that bridges field domain 15 to presentation
# domain 16.
#
# Usage:
#   ./start-select.sh [--config path/to/config.yaml] [--connext-home /path]
#                     [--dry-run]
#
# Options:
#   --config FILE       Selector config (default: scada_select/config.yaml)
...
SELECTOR_BIN="$SCRIPT_DIR/scada_select/build/scada_selector"
DEFAULT_CONFIG="$SCRIPT_DIR/scada_select/config.yaml"
...
if [[ ! -x "$SELECTOR_BIN" ]]; then
    echo >&2 "ERROR: Selector binary not found or not executable: $SELECTOR_BIN"
    echo >&2 "       Build it first: cd scada_select/build && cmake .. && make"
    exit 1
fi
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo "  $SELECTOR_BIN --config $CONFIG ${EXTRA_ARGS[*]+${EXTRA_ARGS[*]}}"
    echo ""
    echo "Environment:"
